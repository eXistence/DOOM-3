...
enum class CameraDrawMode { Wireframe, Solid, Textures };

class RenderCamera {
public:
	// strafing forward/backward. positive distance is forward, negative distance is backward
	void MoveCameraForwardBackward(float distance, bool viewDirection = false) {
		if (!viewDirection) {
			origin = origin + forward * distance;
		} else {
			origin = origin + vpn * distance;
		}
	}

	// strafing left/right. Negative distance is left, positive distance is right
	void MoveCameraLeftRight(float distance) { origin = origin + right * distance; }

	void MoveCameraUpDown(float distance) { origin[2] += distance; }
	...
	void SetOrigin(idVec3 origin) { this->origin = origin; }
	...
	void SetOrigin(float x, float y, float z) { origin = idVec3(x, y, z); }
	...
	CameraDrawMode GetDrawMode() const { return drawMode; }

	void SetDrawMode(CameraDrawMode mode) { drawMode = mode; }

	idAngles GetAngles() const { return angles; }
	...
	idVec3 GetViewRight() const { return vright; }

	idVec3 GetViewUp() const { return vup; }
	...
	const idMat3 &GetRotationMatrix() const { return rotationMatrix; }
...
private:
	...
	;
	idVec3 vright = idVec3(0, 0, 0);
	; // view matrix
	idVec3 forward = idVec3(0, 0, 0);
	;
	idVec3 right = idVec3(0, 0, 0);
	; // move matrix
