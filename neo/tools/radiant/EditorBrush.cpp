...
================
*/
void DrawRenderModel(idRenderModel *model, const idVec3 &origin, const idMat3 &axis, bool cameraView,
					 const idVec3 &color) {
...
static const idVec3 baseaxis[18] = {
	idVec3(0, 0, 1), idVec3(1, 0, 0), idVec3(0, -1, 0),

	// floor
	idVec3(0, 0, -1), idVec3(1, 0, 0), idVec3(0, -1, 0),

	// ceiling
	idVec3(1, 0, 0), idVec3(0, 1, 0), idVec3(0, 0, -1),

	// west wall
	idVec3(-1, 0, 0), idVec3(0, 1, 0), idVec3(0, 0, -1),

	// east wall
	idVec3(0, 1, 0), idVec3(1, 0, 0), idVec3(0, 0, -1),

	// south wall
	idVec3(0, -1, 0), idVec3(1, 0, 0), idVec3(0, 0, -1) // north wall
};
...
================
*/
void Face_MoveTexture(face_t *f, idVec3 delta) { Face_MoveTexture_BrushPrimit(f, delta); }
...
void Face_SetColor(brush_t *b, face_t *f, float fCurveColor) {
	...
	if (g_pParentWnd->GetCamera().GetDrawMode() == CameraDrawMode::Textures &&
		(b->owner && !b->owner->eclass->fixedsize)) {
...
int Brush_Convex(brush_t *b) {
	...
	for (face1 = b->brush_faces; face1; face1 = face1->next) {
		...
		for (face2 = b->brush_faces; face2; face2 = face2->next) {
			...
			if (face1->face_winding->PlanesConcave(*face2->face_winding, face1->plane.Normal(), face2->plane.Normal(),
												   -face1->plane[3], -face2->plane[3])) {
				return false;
			}
		}
	}
...
brush_t *Brush_Parse(idVec3 origin) {
	...
	do {
		...
		if (idStr::Icmp(token, "brushDef") == 0 || idStr::Icmp(token, "brushDef2") == 0 ||
			idStr::Icmp(token, "brushDef3") == 0) {
...
void Brush_Write(brush_t *b, FILE *f, const idVec3 &origin, bool newFormat) {
	...
	for (int j = 0; j < count; j++) {
		WriteFileString(f, "\"%s\" \"%s\"\n", b->epairs.GetKeyVal(j)->GetKey().c_str(),
						b->epairs.GetKeyVal(j)->GetValue().c_str());
	}
...
extern bool RayIntersectsTri(const idVec3 &origin, const idVec3 &direction, const idVec3 &vert0, const idVec3 &vert1,
							 const idVec3 &vert2, float &scale);
...
face_t *Brush_Ray(idVec3 origin, idVec3 dir, brush_t *b, float *dist, bool testPrimitive) {
	...
	if (testPrimitive && !g_PrefsDlg.m_selectByBoundingBrush) {
		...
		} else if (b->modelHandle != NULL && dynamic_cast<idRenderModelPrt *>(b->modelHandle) == NULL &&
				   dynamic_cast<idRenderModelLiquid *>(b->modelHandle) == NULL) {
...
static void DrawSpeaker(const brush_t *b, bool bSelected, bool twoD) {
	...
	if (bSelected) {
		minColor.Set(g_qeglobals.d_savedinfo.colors[COLOR_SELBRUSHES].x,
					 g_qeglobals.d_savedinfo.colors[COLOR_SELBRUSHES].y,
					 g_qeglobals.d_savedinfo.colors[COLOR_SELBRUSHES].z, .5);
	} else {
	...
	if (ret.model->IsDynamicModel() != DM_STATIC) {
		...
		if (dynamic_cast<idRenderModelMD5 *>(ret.model)) {
			if (!b->animSnapshotModel) {
				...
				if (!b->animSnapshotModel) {
					common->Warning("Unknown animation '%s' for classname '%s' (entity %s)", anim, classname,
									b->owner->ValueForKey("name"));
...
================
*/
static void GLTransformedCircle(int type, idVec3 origin, float r, idMat3 mat, float pointSize, idVec3 color,
								float maxDist) {
...
static void Brush_DrawAxis(const brush_t *b) {
	if (g_pParentWnd->ActiveXY()->RotateMode() && b->modelHandle) {
		...
		float xr, yr, zr;
		xr = (b->modelHandle->Bounds()[1].x > b->modelHandle->Bounds()[0].x)
				 ? b->modelHandle->Bounds()[1].x - b->modelHandle->Bounds()[0].x
				 : b->modelHandle->Bounds()[0].x - b->modelHandle->Bounds()[1].x;
		yr = (b->modelHandle->Bounds()[1].y > b->modelHandle->Bounds()[0].y)
				 ? b->modelHandle->Bounds()[1].y - b->modelHandle->Bounds()[0].y
				 : b->modelHandle->Bounds()[0].y - b->modelHandle->Bounds()[1].y;
		zr = (b->modelHandle->Bounds()[1].z > b->modelHandle->Bounds()[0].z)
				 ? b->modelHandle->Bounds()[1].z - b->modelHandle->Bounds()[0].z
				 : b->modelHandle->Bounds()[0].z - b->modelHandle->Bounds()[1].z;
...
static void Brush_DrawEnv(const brush_t *b, bool cameraView, bool bSelected) {
	...
	if (model) {
		...
		DrawRenderModel(model, origin, axis, true,
						bSelected ? g_qeglobals.d_savedinfo.colors[COLOR_SELBRUSHES] : idVec3(1, 1, 1));
...
void Face_Draw(const face_t *f, const idVec4 &color) {
	...
	for (int i = 2; i < f->face_winding->GetNumPoints(); ++i) {
		buffer->Add((*f->face_winding)[0].ToVec3(), (*f->face_winding)[i - 1].ToVec3(), (*f->face_winding)[i].ToVec3(),
					color);
	}
}
...
void Brush_Move(brush_t *b, const idVec3 move, bool bSnap, bool updateOrigin) {	
	...
	if (b->owner->eclass->fixedsize || b->owner->HasModel() ||
		(updateOrigin && b->owner->HasVec3ForKey("origin"))) {
		//		if (!b->entityModel) {
		bool adjustOrigin = true;
		if (b->trackLightOrigin) {
			char text[128];
			b->owner->lightOrigin += move;
			sprintf(text, "%i %i %i", (int)b->owner->lightOrigin[0], (int)b->owner->lightOrigin[1],
...
void Brush_MakeSidedSphere(int sides) {
	...
	for (i = 0; i < sides; i++) {
		for (j = 0; j < sides - 1; j++) {
			...
			f->planepts[0] =
				idPolar3(radius, idMath::TWO_PI * i / sides, idMath::PI * ((float)(j) / sides - 0.5f)).ToVec3() + mid;
			f->planepts[1] =
				idPolar3(radius, idMath::TWO_PI * i / sides, idMath::PI * ((float)(j + 1) / sides - 0.5f)).ToVec3() +
				mid;
			f->planepts[2] =
				idPolar3(radius, idMath::TWO_PI * (i + 1) / sides, idMath::PI * ((float)(j + 1) / sides - 0.5f))
					.ToVec3() +
				mid;
		}
	}
