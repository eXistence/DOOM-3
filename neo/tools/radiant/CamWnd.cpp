...
/*
 ...
 =======================================================================================================================
 */
void CCamWnd::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags) { g_pParentWnd->HandleKey(nChar, nRepCnt, nFlags); }
...
void CCamWnd::OnPaint() {
	...
	if (g_bClipMode) {
		if (g_Clip1.Set() && g_Clip2.Set()) {
			g_pSplitList = ((g_pParentWnd->ActiveXY()->GetViewType() == ViewType::XZ) ? !g_bSwitch : g_bSwitch)
							   ? &g_brBackSplits
							   : &g_brFrontSplits;
		}
	}
...
/*
 ...
 =======================================================================================================================
 */
void CCamWnd::OnDestroy() { CWnd::OnDestroy(); }

/*
 =======================================================================================================================
 =======================================================================================================================
 */
void CCamWnd::OnClose() { CWnd::OnClose(); }
...
/*
 ...
 =======================================================================================================================
 */
void CCamWnd::OnLButtonUp(UINT nFlags, CPoint point) { OriginalMouseUp(nFlags, point); }

/*
 =======================================================================================================================
 =======================================================================================================================
 */
void CCamWnd::OnMButtonDown(UINT nFlags, CPoint point) { OriginalMouseDown(nFlags, point); }

/*
 =======================================================================================================================
 =======================================================================================================================
 */
void CCamWnd::OnMButtonUp(UINT nFlags, CPoint point) { OriginalMouseUp(nFlags, point); }

/*
 =======================================================================================================================
 =======================================================================================================================
 */
void CCamWnd::OnRButtonDown(UINT nFlags, CPoint point) { OriginalMouseDown(nFlags, point); }

/*
 =======================================================================================================================
 =======================================================================================================================
 */
void CCamWnd::OnRButtonUp(UINT nFlags, CPoint point) { OriginalMouseUp(nFlags, point); }
...
void CCamWnd::Cam_MouseDown(int x, int y, int buttons) {
	...
	if ((buttons == MK_LBUTTON) || (buttons == (MK_LBUTTON | MK_SHIFT)) || (buttons == (MK_LBUTTON | MK_CONTROL)) ||
		(buttons == (MK_LBUTTON | MK_CONTROL | MK_SHIFT)) || (buttons == nMouseButton) ||
		(buttons == (nMouseButton | MK_SHIFT)) || (buttons == (nMouseButton | MK_CONTROL)) ||
...
/*
 ...
 =======================================================================================================================
 */
static void setGLMode(CameraDrawMode mode) {
	switch (mode) {
	case CameraDrawMode::Wireframe:
...
void CCamWnd::Cam_Draw() {
	...
	g_qeglobals.sw->PlaceListener(m_Camera->GetOrigin(), m_Camera->GetRotationMatrix(), 0, Sys_Milliseconds(),
								  "Undefined");
	...
	g_splineList->draw(
		static_cast<bool>(g_qeglobals.d_select_mode == sel_addpoint || g_qeglobals.d_select_mode == sel_editpoint));

	if (g_qeglobals.selectObject &&
		(g_qeglobals.d_select_mode == sel_addpoint || g_qeglobals.d_select_mode == sel_editpoint)) {
...
/*
 ...
 =======================================================================================================================
 */
void CCamWnd::OnKeyUp(UINT nChar, UINT nRepCnt, UINT nFlags) { g_pParentWnd->HandleKey(nChar, nRepCnt, nFlags, false); }
	...
	if (v && *v) {
		...
		if (!stricmp(name, v)) {
			...
			if (md5) {
				...
				gameEdit->ANIM_CreateAnimFrame(md5, anim, refent.numJoints, refent.joints, (frame * 1000) / 24, offset,
											   false);
			}
			if (ent->modelDef >= 0) {
				g_qeglobals.rw->FreeEntityDef(ent->modelDef);
			...
			for (int i = 0; i < model->NumSurfaces(); i++) {
				...
				for (int j = 0; j < tri2->numVerts; j++) {
					...
					if (matrix) {
						...
						float y = v[1];
						if (a) {
							float x2 = (((x - brush->owner->origin[0]) * c) - ((y - brush->owner->origin[1]) * s)) +
									   brush->owner->origin[0];
							float y2 = (((x - brush->owner->origin[0]) * s) + ((y - brush->owner->origin[1]) * c)) +
									   brush->owner->origin[1];
							x = x2;
							y = y2;
						}
						v[0] = x;
...
void Select_ToOBJ() {
	int i;
	CFileDialog dlgFile(FALSE, "obj", NULL, OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT,
						"Wavefront object files (*.obj)|*.obj||", g_pParentWnd);
...
idGLDrawablePreview::idGLDrawablePreview() { m_Camera = &g_pParentWnd->GetCamera(); }

idGLDrawablePreview::~idGLDrawablePreview() {}

void idGLDrawablePreview::draw(int x, int y, int w, int h) {

	common->Printf("%d idGLDrawablePreview draw\n", Sys_Milliseconds());
	this->width = w;
	this->height = h;
	...
	const float clearColor[] = {0.5f, 0.1f, 0.1f, 0.0f};
...
void idGLDrawablePreview::Cam_Draw() {
	if (!active_brushes.next) {
		return; // not valid yet
	}
...
/*
 ...
 =======================================================================================================================
 */
fhRenderMatrix idGLDrawablePreview::CreateProjectionMatrix() const {
	float xfov = 90;
	float yfov = 2 * atan((float)height / width) * idMath::M_RAD2DEG;
...
void idGLDrawablePreview::InitCull() {
	const float xfov = 90;
	const float yfov = 2 * atan((float)height / width) * idMath::M_RAD2DEG;
...
bool idGLDrawablePreview::CullBrush(brush_t *b, bool cubicOnly) {
...
void idGLDrawablePreview::Cam_Render() {
	if (!active_brushes.next) {
		return; // not valid yet
	}
...
=================
*/
void idGLDrawablePreview::BuildRendererState() {
	renderEntity_t worldEntity;
	entity_t *ent;
	brush_t *brush;
...
void idGLDrawablePreview::mouseMove(float x, float y) {
	if (!handleMove) {
		prevCursor.x = -1;
		prevCursor.y = -1;
		return;
	...
	if (prevCursor.x > 0 && prevCursor.y > 0) {
		const float sensitiviy = 0.25f;

		const int lr = (int)(prevCursor.x - x) * sensitiviy;
		const int ud = (int)(y - prevCursor.y) * sensitiviy;

		if (lr > 100 || lr < -100) {
			common->Printf("prevx=%f   currentx=%f        lr=%d\n", prevCursor.x, x, lr);
		}
		
		m_Camera->TurnCameraLeftRight(lr);
		m_Camera->TurnCameraUpDown(ud);
